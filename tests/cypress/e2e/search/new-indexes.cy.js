 ...
 * @see tests/specs/search/prod_search_new-indexes.spec.js (wdio)
 */
...
describe('Search China', function () {
  it('should display expected number of results', function () {
    ...
    cy.get(searchCount).invoke('text').then((count) => {
      const countNum = Number(count.replace(/,/, ''))
      expect(countNum).to.be.at.least(20712)
    })
  })
...
describe('Search Tokyo', function () {
  it('should display expected number of results', function () {
    ...
    cy.get(searchCount).invoke('text').then((count) => {
      const countNum = Number(count.replace(/,/, ''))
      expect(countNum).to.be.at.least(5090)
    })
  })
