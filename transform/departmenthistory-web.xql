(:~
    ...
    ODD: /db/apps/hsg-shell/resources/odd/source/departmenthistory.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/departmenthistory/web";
            ...
            return
                    ...
                    case element(figure) return
                        if (head or @rendition='simple:display') then
                            ...
                            html:inline($config, ., ("tei-figure2", "float-left", "figure-floated", css:map-rend-to-class(.)), .)
                    ...
                    case element(pb) return
                        (
                            html:link($config, ., ("tei-pb1", css:map-rend-to-class(.)), concat('[Page ', @n, ']'), (), (), map {"link": ()})
                        )
                        ...
                        if (@rend = 'strong') then
                            ...
                            if (@rend = 'italic') then
                                ...
                                if (@rend = 'smallcaps') then
                                    ...
                                    if (@rendition) then
                                        html:inline($config, ., css:get-rendition(., ("tei-hi4", css:map-rend-to-class(.))), .)
                                    else
                    ...
                    case element(note) return
                        if (@rend = 'inline') then
                            html:paragraph($config, ., ("tei-note1", css:map-rend-to-class(.)), .)
                        else
                            html:note($config, ., ("tei-note2", css:map-rend-to-class(.)), ., "foot", @n/string())
                    ...
                    case element(list) return
                        if (@rend = 'bulleted') then
                            html:list($config, ., ("tei-list1", css:map-rend-to-class(.)), item, ())
                        else
                            if (@type = ('participants', 'to', 'from', 'subject')) then
                                html:list($config, ., ("tei-list2", css:map-rend-to-class(.)), item, ())
                            else
                                if (label) then
                                    html:list($config, ., ("tei-list3", css:map-rend-to-class(.)), item, ())
                                else
                                    html:list($config, ., ("tei-list4", css:map-rend-to-class(.)), item, ())
                    ...
                    case element(docTitle) return
                        if (ancestor::teiHeader) then
                            ...
                            html:block($config, ., css:get-rendition(., ("tei-docTitle2", css:map-rend-to-class(.))), .)
                    ...
                    case element(graphic) return
                        html:graphic($config, ., ("tei-graphic", css:map-rend-to-class(.)), ., xs:anyURI('https://static.history.state.gov/' || $parameters?base-uri ||
                    ...
                    case element(title) return
                        html:inline($config, ., ("tei-title", css:map-rend-to-class(.)), .)
                    ...
                    case element(publicationStmt) return
                        (
                            ...
                            html:block($config, ., ("tei-publicationStmt3", css:map-rend-to-class(.)), date)
                        )
