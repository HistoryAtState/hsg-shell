...
declare variable $config:FRUS_VOLUMES_COL := $config:FRUS_COL || "/volumes";

(: TODO: Create post-install task for `toc:generate-frus-tocs()` to create this folder, if not available  :)
declare variable $config:FRUS_VOLUMES_TOC := $config:FRUS_COL || "/frus-toc/";

declare variable $config:FRUS_METADATA_COL := $config:FRUS_COL || "/bibliography";

declare variable $config:FRUS_METADATA := collection($config:FRUS_METADATA_COL);

declare variable $config:FRUS_CODE_TABLES_COL := $config:FRUS_COL || "/code-tables";
...
declare variable $config:BUILDINGS_COL := $config:OTHER_PUBLICATIONS_COL || "/buildings";
...
declare variable $config:COUNTRIES_ARTICLES_COL := $config:COUNTRIES_COL || "/articles";
declare variable $config:SHORT_HISTORY_COL := $config:OTHER_PUBLICATIONS_COL || "/short-history";
declare variable $config:ADMINISTRATIVE_TIMELINE_COL := $config:OTHER_PUBLICATIONS_COL || "/administrative-timeline";
declare variable $config:ADMINISTRATIVE_TIMELINE_DATA_COL := $config:ADMINISTRATIVE_TIMELINE_COL || "/timeline";
declare variable $config:SECRETARY_BIOS_COL := $config:OTHER_PUBLICATIONS_COL || "/secretary-bios";
declare variable $config:MILESTONES_COL := "/db/apps/milestones";
declare variable $config:MILESTONES_CHAPTERS_COL := $config:MILESTONES_COL || "/chapters";
declare variable $config:EDUCATION_COL := $config:OTHER_PUBLICATIONS_COL || "/education/introductions";
declare variable $config:FAQ_COL := $config:OTHER_PUBLICATIONS_COL || "/faq";
declare variable $config:VIETNAM_GUIDE_COL := $config:OTHER_PUBLICATIONS_COL || "/vietnam-guide";
declare variable $config:VIEWS_FROM_EMBASSY_COL := $config:OTHER_PUBLICATIONS_COL || "/views-from-the-embassy";
...
declare variable $config:PRESIDENTS_COL := "/db/apps/travels/presidents";
...
declare function config:last-modified-from-git ($path-to-xar, $path-to-resource as xs:string) {
        let $_ := util:log("info", "config:last-modified-from-git $path-to-xar: "|| $path-to-xar || " path-to-resource: " || $path-to-resource)
        let $last-modified-date-time := collection($path-to-xar || "/.last-modified")/id(replace($path-to-resource, "/", "0x2F"))/text()
        let $_ := util:log("info", "config:last-modified-from-git-path: " || $last-modified-date-time)
        return
...
declare variable $config:PUBLICATIONS :=
    map {
        ...
        "buildings": map {
            ...
            "document-last-modified": function($document-id) { config:last-modified-from-git($config:OTHER_PUBLICATIONS_COL, "buildings/" || $document-id || '.xml') },              
        ...
        "people": map {
            ...
            "publication-last-modified": config:last-modified-from-repo-xml($config:OTHER_PUBLICATIONS_COL),
            "document-last-modified": function($document-id) { config:last-modified-from-git($config:OTHER_PUBLICATIONS_COL, "secretary-bios/" || $document-id || '.xml') },
            ...
            "breadcrumb-title": 
                function($parameters as map(*)) as xs:string? {
                    ...
                    let $current-country-name := 
                        ...
                        return
                            $trips[last()]/country
                    let $president-name := head(collection($config:PRESIDENTS_COL)//president[id eq $key])/name
                    return
        ...
        "short-history": map {
            ...
            "publication-last-modified":config:last-modified-from-repo-xml($config:OTHER_PUBLICATIONS_COL),
            "document-last-modified": function($document-id) { config:last-modified-from-git($config:OTHER_PUBLICATIONS_COL, "short-history/" || $document-id || '.xml') },
        ...
        "faq": map {
            ...
            "publication-last-modified": config:last-modified-from-repo-xml($config:OTHER_PUBLICATIONS_COL),
            "document-last-modified": function($document-id) { config:last-modified-from-git($config:OTHER_PUBLICATIONS_COL, "faq/" || $document-id || '.xml') },
        ...
        "education": map {
            ...
            "document-last-modified": function($document-id) { config:last-modified-from-git($config:OTHER_PUBLICATIONS_COL, "education/introductions/" || $document-id || '.xml') },  
        ...
        "vietnam-guide": map {
            ...
            "publication-last-modified": config:last-modified-from-repo-xml($config:OTHER_PUBLICATIONS_COL),
            "document-last-modified": function($document-id) { config:last-modified-from-git($config:OTHER_PUBLICATIONS_COL, "vietnam-guide/" || $document-id || '.xml') },
        ...
        "views-from-the-embassy": map {
            ...
            "document-last-modified": function($document-id) { config:last-modified-from-git($config:OTHER_PUBLICATIONS_COL, "views-from-the-embassy/" || $document-id || '.xml') },            
